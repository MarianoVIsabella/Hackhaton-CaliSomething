...
class JudgeAgent:
    ...
    def review(
        ...
        prompt = {
            ...
            "review_rules": [
                "Accept BUY only if trend is up, news is positive, and confidence is reasonably high.",
                "Accept SELL only if trend is down and either news is negative or an existing position should be reduced.",
                "Change to HOLD if signals conflict.",
                "Change to HOLD if required data is missing.",
